...
def generate_xpart_particles(config_dict, line, ref_particle, start_elem, capacity):
    ...
    particles = _prepare_matched_beam(config_dict, line, ref_particle, start_elem,  
...
def prepare_particles(config_dict, line, ref_particle, start_elem):
    capacity = config_dict['run']['max_particles']

    particles = generate_xpart_particles(config_dict, line, ref_particle, start_elem, capacity)
...
def execute(config_file_path, config_dict):
    ...
    line, ref_part, bgman, start_elem, s0 = load_and_process_line(config_dict)
    build_collimation_tracker(line)

    particles = prepare_particles(config_dict, line, ref_part, start_elem)
